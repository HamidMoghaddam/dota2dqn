...
local function BitAND(a,b)--Bitwise and
 ...
 function CAddonTemplateGameMode:ParseHeroLevelUp(eHero, reply)	
...
-- Main entry function --
 function CAddonTemplateGameMode:ParseHeroCommand(eHero, reply)	
	local result = package.loaded['game/dkjson'].decode(reply)
...
-- Buying items --
function CAddonTemplateGameMode:Buy(eHero, result)
...
function CAddonTemplateGameMode:Sell(eHero, result)
...
function CAddonTemplateGameMode:UseItem(eHero, result)
	local slot = result.slot
	local eItem = eHero:GetItemInSlot(slot)
	if eItem then
 ...
 function CAddonTemplateGameMode:Noop(eHero, result)
	--Noop
end
 
function CAddonTemplateGameMode:MoveTo(eHero, result)
	eHero:MoveToPosition(Vector(result.x, result.y, result.z))
	Say(nil, eHero:GetName() .. " moving to " .. result.x ..", " .. result.y ..", " .. result.z, false)
end

function CAddonTemplateGameMode:Attack(eHero, result)
...
function CAddonTemplateGameMode:Cast(eHero, result)
	local eAbility = eHero:GetAbilityByIndex(result.ability)
	self:UseAbility(eHero, eAbility)		
end

function CAddonTemplateGameMode:UseAbility(eHero, eAbility)
